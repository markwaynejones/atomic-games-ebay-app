...
var allPsoneGames = getAllPsoneGames();
  ...
  it("should check that site title of webdriverio", function() {
    ...
    mysql
      ...
      .then(function(rows) {
        ...
        // console.log("type of: " + typeof rows[0]);
        if (typeof rows[0] != "undefined") {
          dateOfLastSOldItemInDB = rows[0].datetimesold;
        } else {
          dateOfLastSOldItemInDB = 0;
        }
        // console.log("game name 123: " + dateOfLastSOldItemInDB);
    ...
    // browser.pause(10000); // have to pause this long to get timestamp
    ...
    */
    ...
    // var resultMark = databaseConnection.query(
    //   // "SELECT * from `atomic-games`.`sold-playstation-one-games` order by `datetimesold` desc limit 1"
    //   // "SELECT * from `atomic-games`.`sold-playstation-one-games` order by `datetimesold`"
    //   "SELECT `datetimesold` from `sold-playstation-one-games`"
    // );
    // browser.pause(3000);
    // // console.log("lastest timestamp: " + result.datetimesold);
    // console.log("markname: " + resultMark[0]);
    ...
    for (index = 0; index < gameTitles.value.length; ++index) {
      // var result = databaseConnection.query(
      //   "select `datetimesold` from `sold-playstation-one-games` order by `datetimesold` desc limit 1"
      // );

      // console.log("lastest timestamp: " + result[0]);
      ...
      var postageCost = gameTitles.value[index]
        .element(
      ...
      var numberOfBids = gameTitles.value[index]
        .element(
      ...
      var imageSrc = gameTitles.value[index]
      ...
      ///////// start of checking game heading //////////

      // start replace special characters
      // gameHeadingReplacedSpecialChars = gameHeading.replace(",", "");
      // gameHeadingReplacedSpecialChars = gameHeadingReplacedSpecialChars.replace(
      //   ":",
      //   ""
      // );
      // gameHeadingReplacedSpecialChars = gameHeadingReplacedSpecialChars.replace(
      ...
      //   " "
      // );
      // gameHeadingReplacedSpecialChars = gameHeading.replace("2", "");
      // gameHeadingReplacedSpecialChars = gameHeadingReplacedSpecialChars.replace(
      //   "3",
      //   ""
      // );
      ...
      gameHeadingReplacedSpecialChars = gameHeading;
      ...
      for (var i = 0; i < allPsoneGames.length; i++) {
        ...
        // does rayman (currentGame) appear anywhere in the game heading (gameHeadingReplacedSpecialChars)
        if (
          currentGame == "Crash Bandicoot 2" ||
          currentGame == "Crash Bandicoot Warped"
        ) {
          console.log("does this string: " + gameHeadingReplacedSpecialChars);
          console.log("include this string: " + currentGame);
          console.log(
            "currentGameAppearsInEbayTitle: " + currentGameAppearsInEbayTitle
          );
        }

        // if it does appear then dont insert into db and move onto next ebay sold item/game
        if (currentGameAppearsInEbayTitle === true) {
          console.log(
            "This game is stopping it being inserted into DB: " + currentGame
          );
